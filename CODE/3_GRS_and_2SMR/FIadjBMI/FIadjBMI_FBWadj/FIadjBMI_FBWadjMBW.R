...
fbw_2_fiadjbmi <- fbw[which(fbw$RSID%in%fiadjbmi$rsID),] #28/31!! So close.

#Let's check if with chr_pos we can get some others...

fbw$chr_pos <- paste("chr", fbw$chr, ":", fbw$pos, sep = "")

fbw_2_fiadjbmi_2 <- fbw[which(fbw$chr_pos%in%fiadjbmi$chr_pos),] #32 - same.

#Let's check the MAFs if fbw_2_fiadjbmi_2:

summary(fbw_2_fiadjbmi_2$eaf) #they are OK. We did great at conserving that SNP.
...
missing_snps <- fiadjbmi[which(!(fiadjbmi$chr_pos%in%fbw_2_fiadjbmi_2$chr_pos)),] #3

#Query them in LDlink and check whether they are in 1000G

setwd("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CODE/3_2SMR_and_GRS/FIadjBMI/FIadjBMI_FBWadj/proxies/")
...
proxies_ld_link <- fread("combined_query_snp_list.txt")
...
#We are going to match first the proxies in fbw and then work with the mismatches.

proxies_in_FBW <- fbw[which(fbw$chr_pos%in%proxies_df_clean_curated$Coord),] #247

#Let's match those found in FBW with FIadjBMI:

proxies_in_FIadjBMI <- proxies_in_FIadjBMI[which(proxies_in_FIadjBMI$chr_pos%in%proxies_in_FBW$chr_pos),] #247/247!! Perfect match

#Let's check just in case...

missing_snp <- proxies_in_FBW[which(!(proxies_in_FBW$chr_pos%in%proxies_in_FIadjBMI$chr_pos)),] #none. It is a duplicate.
dupl_snp <- proxies_in_FBW$chr_pos[which(duplicated(proxies_in_FBW$chr_pos) == TRUE)] #none

#AWESOME.
#We've got the proxies that we want: 247.
#The next thing to do is easy: let's get some clumping done.
...
proxies_df_check <- proxies_df_clean[which(proxies_df_clean$Coord%in%proxies_in_FBW$chr_pos),]

check_fiadjbmi <- proxies_in_FIadjBMI
check_fbw <- proxies_in_FBW
...
#3. Let's check the alleles for FBW and see if the mismatches are due to formating and not due to wrong SNP.

check_fbw$ea <- as.character(unlist(sapply(check_fbw$ea, toupper)))
check_fbw$nea <- as.character(unlist(sapply(check_fbw$nea, toupper)))

check_fbw$alleles_1 <- paste("(", check_fbw$ea, "/", check_fbw$nea, ")", sep = "")
check_fbw$alleles_2 <- paste("(", check_fbw$nea, "/", check_fbw$ea, ")", sep = "")

#And now check everything!

index_vect <- c()

for(i in seq(1,length(check_fbw$chr_pos))){
  
  print(i)
  
  index_tmp <- as.numeric(as.character(unlist(sapply(check_fbw$chr_pos[i], match_ss_data, query_alleles_1 = check_fbw$alleles_1[i], query_alleles_2 = check_fbw$alleles_2[i], bim_data = proxies_df_check))))
...
fiadjbmi_in_fbw <- fiadjbmi[which(fiadjbmi$chr_pos%in%fbw_2_fiadjbmi_2$chr_pos),]

fiadjbmi_in_fbw_4_clumping <- fiadjbmi_in_fbw %>%
  select(rsID, P)

colnames(fiadjbmi_in_fbw_4_clumping) <- c("rsid", "pval")
...
variants_4_clumping <- rbind(proxies_4_clumping, fiadjbmi_in_fbw_4_clumping)
...
independence_test_end[which(!(independence_test_end$rsid%in%fiadjbmi_in_fbw_4_clumping$rsid)),]
...
proxies_recovered <- c("chr2:622827", "chr1:219668252") #chr_pos obtain from checking in FBW which has both. WHAT A PAIN.
...
exposure_fiadjbmi_1 <- fiadjbmi_in_fbw %>%
...
#And now we just need to get the variants from FBW and get the data too:

outcome_fbw <- fbw[which(fbw$RSID%in%exposure_fiadjbmi$SNP),]

outcome_fbw <- outcome_fbw %>%
  select(RSID, ea,nea, eaf, beta, se, p, n_offBW) #fetal effect adjusted by mother's so.... we use offspring samplesize.
...
#2. We do not have INFO data, but we can get the samplesize:

FIadjBMI_original_exposure_ordered <- FIadjBMI_original[which(FIadjBMI_original$chr_pos%in%exposure_fiadjbmi$chr_pos),]

FIadjBMI_original_exposure_ordered <- FIadjBMI_original_exposure_ordered[order(match(FIadjBMI_original_exposure_ordered$chr_pos, exposure_fiadjbmi$chr_pos)),]

length(which(FIadjBMI_original_exposure_ordered$chr_pos == exposure_fiadjbmi$chr_pos))

exposure_fiadjbmi$samplesize.exposure <- FIadjBMI_original_exposure_ordered$sample_size

#3. Let's check for MHC:
...
summary(as.numeric(outcome_fbw$eaf.outcome)) #perfect
...
outcome_fbw$id.outcome <- "Fetal effect on own birth weight adjusted for maternal effect"
outcome_fbw$outcome <- "Fetal effect on own birth weight adjusted for maternal effect"

dat_1_pre <- harmonise_data(exposure_fiadjbmi_pos, outcome_fbw, action = 3) #30
...
saveRDS(dat_1_filt_clean, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/3_GRS_output/FIadjBMI/FBWadjMBW/FIadjBMI_FBWadjMBW_merged_df")
...
saveRDS(grs_weighted, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/3_GRS_output/FIadjBMI/FBWadjMBW/FIadjBMI_FBWadjMBW_weighted")
saveRDS(grs_unweighted, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/3_GRS_output/FIadjBMI/FBWadjMBW/FIadjBMI_FBWadjMBW_unweighted")
...
dat_1_filt_steiger <- dat_1_filt_steiger[which(dat_1_filt_steiger$steiger_dir == TRUE),] #27/30. Not bad!!

dat_1_filt_steiger$mr_keep <- TRUE #this allows us to keep the unambiguous.

saveRDS(dat_1_filt_steiger, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/FIadjBMI/FBWadjMBW/FIadjBMI_FBWadjMBW_merged_df")

dat_1_filt_steiger <- readRDS("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/FIadjBMI/FBWadjMBW/FIadjBMI_FBWadjMBW_merged_df")

#Let's check the weird ones:
...
saveRDS(mr_results, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/FIadjBMI/FBWadjMBW/FIadjBMI_FBWadjMBW_original")
...
saveRDS(rucker, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/FIadjBMI/FBWadjMBW/FIadjBMI_FBWadjMBW_rucker_test_original")
...
saveRDS(het_Q_Isq, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/FIadjBMI/FBWadjMBW/FIadjBMI_FBWadjMBW_Q_Isq_original")

#3: Sensitivity plots:

dat_1_filt_steiger$labels <- NA

tiff("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/sensitivity_plots/IR/FIadjBMI_FBWadjMBW_original.tiff ", units="in", width=10, height=10, res=300)
...
#rs116141873: Birthweight! It truly is an outlier
#rs2943646: Impedance of right/left arm + whole body (4th: log FIadjBMI) -> most negative driving the association

dat_1_post <- dat_1_filt_steiger[-which(dat_1_filt_steiger$SNP%in%outliers),]

saveRDS(dat_1_post, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/FIadjBMI/FBWadjMBW/FIadjBMI_FBWadjMBW_merged_df_post_outliers")
...
saveRDS(mr_results, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/FIadjBMI/FBWadjMBW/FIadjBMI_FBWadjMBW_post")
...
saveRDS(rucker, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/FIadjBMI/FBWadjMBW/FIadjBMI_FBWadjMBW_rucker_test_post")
...
saveRDS(het_Q_Isq, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/FIadjBMI/FBWadjMBW/FIadjBMI_FBWadjMBW_Q_Isq_post")

#3: Sensitivity plots:

dat_1_post$labels <- NA

tiff("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/sensitivity_plots/IR/FIadjBMI_FBWadjMBW_post.tiff ", units="in", width=10, height=10, res=300)
mr_plots(dat_1_post)
dev.off()
...
saveRDS(mr_clust_res, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/FIadjBMI/FBWadjMBW/FIadjBMI_FBWadjMBW_MR_CLUST")
