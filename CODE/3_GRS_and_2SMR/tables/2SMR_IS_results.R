...
clean_mr_res <- function(mr_df){
  
  #We just need to make a fake dataframe...
  
  egger_df <- mr_df
  wmed_df <- mr_df
  wmod_df <- mr_df
  
  #Let's change some stuff here too ;)
  
  egger_df$method <- "MR Egger"
  wmed_df$method <- "Weighted median"
  wmod_df$method <- "Weighted mode"
  
  #OK, let's go and change the others
  ...
  #Finally, let's get the final dataframe:
  
  final_df <- rbind(mr_df, egger_df, wmed_df, wmod_df)
...
mr_file_finder <- function(list_of_files, path_, exposure, outcome){
  ...
  #We have some cases in which only IVW could be done
  #since we have <3 SNPs.
  #In those cases we need to clean the data a bit.
    
  if(length(final_df$nsnp) == 1){ #This will order the data properly :)
      
      final_df <- clean_mr_res(final_df)
...
#  #In those cases we need to clean the data a bit.
#  
#  if(length(final_df$nsnp) == 1){
#    
#    final_df <- clean_mr_res(final_df)
#    
#  }
#  
#  #Now let's arrange the columns according to how we want them to be displayed
#  #in the tables...
#  
#  if(outcome == "FBW"){
#  
...
#  
#  #Finally, we are going to do some tricks to make the data beautiful
#  #So that we can save it properly, but only for FIadjBMI, which is the first one:
#  
#  if(exposure == "CIR"){
#  
#     final_df_clean_copy <- final_df_clean
...
IS_FBW <- mr_results_parser("FBW")
IS_FBWadjMBW <- mr_results_parser("FBWadjMBW")
IS_MBW <- mr_results_parser("MBW")
IS_MBWadjFBW <- mr_results_parser("MBWadjFBW")
IS_Father <- mr_results_parser("Father")

IS_df <- cbind(IS_FBW, IS_FBWadjMBW, IS_MBW, IS_MBWadjFBW, IS_Father)

#We save these fellas:

fwrite(IS_df, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/Manuscript/supplementary_tables_per_method/GRS_&_2SMR/4_2SMR_results/2SMR_IS_results_updated.csv", sep = ",", col.names = TRUE, row.names = FALSE, na = "-")
