...
IR_53 <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CURATED_DATA/Data_4_2SMR/53_IR_Lotta/CLEAN_53_LOTTA_GRS_SNPs_Manuscript.txt")
mbw <- fread("C:/Users/zlc436/Desktop/Liftover/OUTPUT/MBW/Birthweight_offspring_mothers2021_curated_FULL.txt")
...
mbw_2_IR_53 <- mbw[which(mbw$rsID%in%IR_53$rsID),] #53/53
...
#Now we do the same for IR_53:

#Now we do the same for IR_53:

exposure_IR_53 <- IR_53 %>%
  select(rsID, Effect_Allele, Other_Allele, EAF, Beta, SE, P, N)

colnames(exposure_IR_53) <- c("SNP", "effect_allele.exposure", "other_allele.exposure",
...
summary(exposure_IR_53$eaf.exposure) #perfect!
...
summary(as.numeric(outcome_mbw$eaf.outcome)) #there is one NA, but since the MAF in exposure is OK we are gonna keept it.

outcome_mbw$eaf.outcome <- as.numeric(outcome_mbw$eaf.outcome)/100

summary(as.numeric(outcome_mbw$eaf.outcome)) #all in order, but wildly different max of frequencies. Maybe due to different alleles???

#2. Let's check the INFO:

summary(outcome_mbw$info.outcome) #all really high values. PERFECT.

#3. Also the heterogeneity, though that value is in the original data:

summary(as.numeric(mbw_2_IR_53$I2)) #we have some of them with huge heterogeneity.
summary(as.numeric(mbw_2_IR_53$`P-het`)) #But only very few with significant heterogenety values.
...
new_a1 <- ifelse(as.numeric(exposure_IR_53$beta.exposure) < 0, exposure_IR_53$other_allele.exposure, exposure_IR_53$effect_allele.exposure)
new_a2 <- ifelse(as.numeric(exposure_IR_53$beta.exposure) < 0, exposure_IR_53$effect_allele.exposure, exposure_IR_53$other_allele.exposure)
new_beta <- ifelse(as.numeric(exposure_IR_53$beta.exposure) < 0, (-1)*(exposure_IR_53$beta.exposure), exposure_IR_53$beta.exposure)
new_eaf <- ifelse(as.numeric(exposure_IR_53$beta.exposure) < 0, 1-exposure_IR_53$eaf.exposure, exposure_IR_53$eaf.exposure)

exposure_IR_53_pos <- exposure_IR_53

exposure_IR_53_pos$effect_allele.exposure <- new_a1
exposure_IR_53_pos$other_allele.exposure <- new_a2
exposure_IR_53_pos$beta.exposure <- new_beta
exposure_IR_53_pos$eaf.exposure <- new_eaf

#Now let's harmonise the data taking that into account:

exposure_IR_53_pos$id.exposure <- "Insulin resistance"
exposure_IR_53_pos$exposure <- "Insulin resistance"

outcome_mbw$id.outcome <- "Maternal effect on offspring Birth Weight"
outcome_mbw$outcome <- "Maternal effect on offsptring Birth Weight"

dat_1_pre <- harmonise_data(exposure_IR_53_pos, outcome_mbw, action = 3) #15: perfect.
...
saveRDS(dat_1_filt_clean, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/3_GRS_output/53_IR_Lotta_SNPs/MBW/IR_53_MBW_merged_df")
...
dat_1_filt_clean$samplesize.outcome <- 270002
...
saveRDS(grs_weighted, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/3_GRS_output/53_IR_Lotta_SNPs/MBW/IR_53_MBW_weighted")
saveRDS(grs_unweighted, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/3_GRS_output/53_IR_Lotta_SNPs/MBW/IR_53_MBW_unweighted")
...
saveRDS(dat_1_filt_steiger, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/53_IR_Lotta_SNPs/MBW/IR_53_MBW_merged_df")

dat_1_filt_steiger <- readRDS("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/53_IR_Lotta_SNPs/MBW/IR_53_MBW_merged_df")
...
index_10 <- which(F_ < 10)

dat_1_filt_steiger <- dat_1_filt_steiger[-index_10,]
...
saveRDS(mr_results, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/53_IR_Lotta_SNPs/MBW/IR_53_MBW_original")
...
#Huge pleiotropy! This time Egger is chosen.

saveRDS(rucker, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/53_IR_Lotta_SNPs/MBW/IR_53_MBW_rucker_test_original")
...
saveRDS(het_Q_Isq, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/53_IR_Lotta_SNPs/MBW/IR_53_MBW_Q_Isq_original")

#3: Sensitivity plots:

dat_1_filt_steiger$labels <- NA

tiff("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/sensitivity_plots/IR/IR_53_MBW_original.tiff ", units="in", width=10, height=10, res=300)
...
#[1] "rs10195252" "rs12525532" "rs2126259"  "rs4846565"  "rs7005992" 
#[6] "rs7973683"  "rs8101064" 

dat_1_post <- dat_1_filt_steiger[-which(dat_1_filt_steiger$SNP%in%outliers),]

saveRDS(dat_1_post, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/53_IR_Lotta_SNPs/MBW/IR_53_MBW_merged_df_post_outliers")
...
saveRDS(mr_results, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/53_IR_Lotta_SNPs/MBW/IR_53_MBW_post")
...
#Radial MR could not take pleiotropy so good.

rucker_post <- mr_rucker(dat_1_post)

saveRDS(rucker_post, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/53_IR_Lotta_SNPs/MBW/IR_53_MBW_rucker_test_post")
...
saveRDS(het_Q_Isq, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/53_IR_Lotta_SNPs/MBW/IR_53_mbw_Q_Isq_post")

#3: Sensitivity plots:

dat_1_post$labels <- NA

tiff("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/sensitivity_plots/IR/IR_53_mbw_post.tiff ", units="in", width=10, height=10, res=300)
mr_plots(dat_1_post)
dev.off()

#YOU CAN CLEARLY SEE THAT ALL OF THEM ARE JUST SHITTY, BUT THERE IS ONE THAT REALLY IS DRIVING THE ASSOCIATION.
#IF MR-CLUST IS NOT SHOWING THIS, I WILL RUN THE ANALYSIS ON THIS ALONE.
...
saveRDS(mr_clust_res, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/OUTPUT_DATA/4_2SMR_output/53_IR_Lotta_SNPs/MBW/IR_53_MBW_MR_CLUST")

#There are some variants in cluster_1.

cluster_1_rsid <- res_em$results$best$observation[which(res_em$results$best$cluster_class == 1)]

#Both of them have stronger negative theta effects. And the probability of beloning to the cluster is really high.
#Let's put them together and check if there is heterogeneity between them.

dat_1_cluster_1 <- dat_1_filt_steiger[which(dat_1_filt_steiger$SNP%in%cluster_1_rsid),]
