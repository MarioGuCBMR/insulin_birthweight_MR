...
#This is code to run CAUSE for fi -> Father
...
fi_missing_match_1 <- fi_missing[which(fi_missing$rsid%in%Father_$rsID),] #307!!!Quite many. That is great
Father_missing_match_1 <- Father_[which(Father_$rsID%in%fi_missing$rsid),]

#Now I understand, these are only available in build 38 or 18.
#Well that makes a lot of sense. 
#Let's see if we have some match with merged_rsID:

fi_missing_match_2 <- fi_missing[which(fi_missing$rsid%in%Father_$merged_rsID),] #3
Father_missing_match_2 <- Father_[which(Father_$merged_rsID%in%fi_missing$rsid),] #3
...
X <- gwas_merge(fi_not_missing, Father_, snp_name_cols = c("chr_pos_37", "chr_pos_37"), 
...
X_missing_1 <- gwas_merge(fi_missing_match_1, Father_missing_match_1, snp_name_cols = c("rsid", "rsID"), 
                          beta_hat_cols = c("beta", "Beta-A1"), 
                          se_cols = c("se", "SE"), 
                          A1_cols = c("a2", "A1"), 
                          A2_cols = c("a1", "A0"))

X_missing_2 <- gwas_merge(fi_missing_match_2, Father_missing_match_2, snp_name_cols = c("rsid", "rsID"), 
                          beta_hat_cols = c("beta", "Beta-A1"), 
                          se_cols = c("se", "SE"), 
                          A1_cols = c("a2", "A1"), 
                          A2_cols = c("a1", "A0"))
