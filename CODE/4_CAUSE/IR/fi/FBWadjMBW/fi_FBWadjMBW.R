...
#This is code to run CAUSE for fi -> FBWadjMBW
...
fi_missing_match_1 <- fi_missing[which(fi_missing$rsid%in%FBWadjMBW_$RSID),] #22! Not too many
FBWadjMBW_missing_match_1 <- FBWadjMBW_[which(FBWadjMBW_$RSID%in%fi_missing$rsid),]
...
which(swapping_snps_old_2_new$new_rs%in%FBWadjMBW_$RSID) #several need to be changed.
which(swapping_snps_new_2_old$old_rs%in%FBWadjMBW_$RSID) #we need to change some of these fellas too.
...
FBWadjMBW_missing_match_2 <- FBWadjMBW_[which(FBWadjMBW_$RSID%in%fi_missing_match_2$rsid),]

#Since here we do have some mismatches with FBWadjMBW, we are going to do it this way:

mismatch <- swapping_snps_new_2_old[which(swapping_snps_new_2_old$old_rs%in%FBWadjMBW_$RSID),]
...
FBWadjMBW_missing_match_3 <- FBWadjMBW_[which(FBWadjMBW_$RSID%in%fi_missing_match_3$rsid),]
...
X <- gwas_merge(fi_not_missing, FBWadjMBW_, snp_name_cols = c("chr_pos_37", "chr_pos"), 
...
X_missing_1 <- gwas_merge(fi_missing_match_1, FBWadjMBW_missing_match_1, snp_name_cols = c("rsid", "RSID"), 
                          beta_hat_cols = c("beta", "beta"), 
                          se_cols = c("se", "se"), 
                          A1_cols = c("a2", "ea"), 
                          A2_cols = c("a1", "nea"))

X_missing_2 <- gwas_merge(fi_missing_match_2, FBWadjMBW_missing_match_2, snp_name_cols = c("rsid", "RSID"), 
                          beta_hat_cols = c("beta", "beta"), 
                          se_cols = c("se", "se"), 
                          A1_cols = c("a2", "ea"), 
                          A2_cols = c("a1", "nea"))

X_missing_3 <- gwas_merge(fi_missing_match_3, FBWadjMBW_missing_match_3, snp_name_cols = c("rsid", "RSID"), 
                          beta_hat_cols = c("beta", "beta"), 
                          se_cols = c("se", "se"), 
                          A1_cols = c("a2", "ea"), 
                          A2_cols = c("a1", "nea"))
...
FBWadjMBW_match_no_rsid <- FBWadjMBW_[which(FBWadjMBW_$chr_pos%in%fi_match_no_rsid$chr_pos_37),] #WE HAVE THEM.

#Now let's order them and check which ones can we retrieve there.

FBWadjMBW_match_no_rsid <- FBWadjMBW_match_no_rsid[order(match(FBWadjMBW_match_no_rsid$chr_pos, fi_match_no_rsid$chr_pos_37)),]

length(which(FBWadjMBW_match_no_rsid$chr_pos == fi_match_no_rsid$chr_pos_37)) #DONE.

#Let's be wary and get an index of all of those that have rsids:

index_rsid <- which(str_detect(FBWadjMBW_match_no_rsid$RSID, "rs") == TRUE) #ALL OF THEM

length(which(FBWadjMBW_match_no_rsid$chr_pos == fi_match_no_rsid$chr_pos_37)) #DONE. 

fi_match_no_rsid$rsid <- FBWadjMBW_match_no_rsid$RSID
