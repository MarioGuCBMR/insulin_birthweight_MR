...
#This is code to run CAUSE for fi -> FBW
...
fi_missing_match_1 <- fi_missing[which(fi_missing$rsid%in%FBW_$rsID),] #311!!!Quite many. That is great
FBW_missing_match_1 <- FBW_[which(FBW_$rsID%in%fi_missing$rsid),]

#Now I understand, these are only available in build 38 or 18.
#Well that makes a lot of sense. 
#Let's see if we have some match with merged_rsID:

fi_missing_match_2 <- fi_missing[which(fi_missing$rsid%in%FBW_$merged_rsID),] #3
FBW_missing_match_2 <- FBW_[which(FBW_$merged_rsID%in%fi_missing$rsid),] #3
...
X <- gwas_merge(fi_not_missing, FBW_, snp_name_cols = c("chr_pos_37", "chr_pos_37"), 
...
X_missing_1 <- gwas_merge(fi_missing_match_1, FBW_missing_match_1, snp_name_cols = c("rsid", "rsID"), 
                          beta_hat_cols = c("beta", "Beta-A1"), 
                          se_cols = c("se", "SE"), 
                          A1_cols = c("a2", "A1"), 
                          A2_cols = c("a1", "A0"))

X_missing_2 <- gwas_merge(fi_missing_match_2, FBW_missing_match_2, snp_name_cols = c("rsid", "rsID"), 
                          beta_hat_cols = c("beta", "Beta-A1"), 
                          se_cols = c("se", "SE"), 
                          A1_cols = c("a2", "A1"), 
                          A2_cols = c("a1", "A0"))
...
saveRDS(X_end, file = "/home/projects/ku_00095/people/marure/BW_project/CODE/4_CAUSE/IR/fi/FBW/dataframes/X_fi_FBW.rds")
saveRDS(params, file = "/home/projects/ku_00095/people/marure/BW_project/CODE/4_CAUSE/IR/fi/FBW/dataframes/params_fi_FBW.rds")
...
saveRDS(top_vars, "/home/projects/ku_00095/people/marure/BW_project/CODE/4_CAUSE/IR/fi/FBW/dataframes/pruned_fi_FBW.res")
...
saveRDS(res, file = "/home/projects/ku_00095/people/marure/BW_project/CODE/4_CAUSE/IR/fi/FBW/output/res_fi_FBW.rds")

res_strict <- cause(X=X_end, variants = top_vars, param_ests = params, qalpha = 1, qbeta = 2)

saveRDS(res, file = "/home/projects/ku_00095/people/marure/BW_project/CODE/4_CAUSE/IR/fi/FBW/output/res_fi_FBW_strict.rds")
...
tiff("/home/projects/ku_00095/people/marure/BW_project/CODE/4_CAUSE/IR/fi/FBW/output/res_fi_FBW_plot.tiff", units="in", width=1200, height=500, res=300)
plot(res, type="data")
dev.off()
