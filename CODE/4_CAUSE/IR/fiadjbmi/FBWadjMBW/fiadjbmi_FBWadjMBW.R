...
#This is code to run CAUSE for fiadjbmi -> FBWadjMBW
...
#########################
#Cleaning FBWadjMBW data#
#########################

summary(FBWadjMBW$eaf) #Perfect

#Now let's merge the data:

FBWadjMBW_maf <- FBWadjMBW

#No other info is here!!

FBWadjMBW_ <- FBWadjMBW_maf
...
fiadjbmi_$chr_pos <- paste("chr", fiadjbmi_$chr_pos, sep = "")

X <- gwas_merge(fiadjbmi_, FBWadjMBW_, snp_name_cols = c("chr_pos", "chr_pos"), 
...
fiadjbmi_match <- fiadjbmi_[which(fiadjbmi_$chr_pos%in%X$snp),]

fiadjbmi_match <- fiadjbmi_match[order(match(fiadjbmi_match$chr_pos, X$snp)),]

length(fiadjbmi_match$chr_pos == X$snp) #all of them

#Let's check how are we going to work with this:

check <- fiadjbmi_match[order(fiadjbmi_match$RSID),] #do we have any SNPs without RSID?
...
length(which(fiadjbmi_match$RSID == "-")) #not too many!!

fiadjbmi_match_no_rsid <- fiadjbmi_match[which(fiadjbmi_match$RSID == "-"),]
fiadjbmi_match_rsid <- fiadjbmi_match[which(fiadjbmi_match$RSID != "-"),]

length(which(str_detect(fiadjbmi_match_rsid$RSID, "rs") == TRUE)) #not all of them, but most.

weird_ones <- fiadjbmi_match_rsid[which(str_detect(fiadjbmi_match_rsid$RSID, "rs") == FALSE),] #expected ones. Those from Hb1AC.

fiadjbmi_match_rsid <- fiadjbmi_match_rsid[which(str_detect(fiadjbmi_match_rsid$RSID, "rs") == TRUE),] #expected ones. Those from Hb1AC.

#Let's check these SNPs in FBW_, maybe they have RSID there:

fiadjbmi_match_no_rsid <- rbind(fiadjbmi_match_no_rsid, weird_ones)

FBWadjMBW_match_no_rsid <- FBWadjMBW_[which(FBWadjMBW_$chr_pos%in%fiadjbmi_match_no_rsid$chr_pos),] #WE HAVE THEM.

#Now let's order them and check which ones can we retrieve there.

FBWadjMBW_match_no_rsid <- FBWadjMBW_match_no_rsid[order(match(FBWadjMBW_match_no_rsid$chr_pos, fiadjbmi_match_no_rsid$chr_pos)),]

length(which(FBWadjMBW_match_no_rsid$chr_pos == fiadjbmi_match_no_rsid$chr_pos)) #DONE.

#Let's be wary and get an index of all of those that have rsids:

index_rsid <- which(str_detect(FBWadjMBW_match_no_rsid$RSID, "rs") == TRUE) #most of them!!

FBWadjMBW_match_no_rsid_recovered <- FBWadjMBW_match_no_rsid[index_rsid,]
fiadjbmi_match_no_rsid_recovered <- fiadjbmi_match_no_rsid[index_rsid,]

length(which(FBWadjMBW_match_no_rsid_recovered$chr_pos == fiadjbmi_match_no_rsid_recovered$chr_pos)) #DONE. 

fiadjbmi_match_no_rsid_recovered$RSID <- FBWadjMBW_match_no_rsid_recovered$RSID

#AWESOME: now let's be wary with the others:

FBWadjMBW_match_no_rsid_missed <- FBWadjMBW_match_no_rsid[-index_rsid,]
fiadjbmi_match_no_rsid_missed <- fiadjbmi_match_no_rsid[-index_rsid,]

#Let's try with phenoscanner:

ps_results_1 <- phenoscanner::phenoscanner(FBWadjMBW_match_no_rsid_missed$chr_pos)

ps_end <- ps_results_1$snps

#We can use these fellas.
#As for the other SNPs, I think it is OK if we miss 40 SNPs, honestly.
#We did as great as we could!!

FBWadjMBW_match_no_rsid_missed_recovered <- FBWadjMBW_match_no_rsid_missed[which(FBWadjMBW_match_no_rsid_missed$chr_pos%in%ps_end$hg19_coordinates),]
fiadjbmi_match_no_rsid_missed_recovered <- fiadjbmi_match_no_rsid_missed[which(fiadjbmi_match_no_rsid_missed$chr_pos%in%ps_end$hg19_coordinates),]

ps_end <- ps_end[order(match(ps_end$hg19_coordinates, FBWadjMBW_match_no_rsid_missed_recovered$chr_pos)),]

length(which(ps_end$hg19_coordinates == FBWadjMBW_match_no_rsid_missed_recovered$chr_pos))
length(which(ps_end$hg19_coordinates == fiadjbmi_match_no_rsid_missed_recovered$chr_pos))

fiadjbmi_match_no_rsid_missed_recovered$RSID <- ps_end$rsid

#Finally, let's get those that are not there and put a "-"

FBWadjMBW_match_no_rsid_missed_missed <- FBWadjMBW_match_no_rsid_missed[which(!(FBWadjMBW_match_no_rsid_missed$chr_pos%in%ps_end$hg19_coordinates)),]
fiadjbmi_match_no_rsid_missed_missed <- fiadjbmi_match_no_rsid_missed[which(!(fiadjbmi_match_no_rsid_missed$chr_pos%in%ps_end$hg19_coordinates)),]
...
fiadjbmi_match_end <- fiadjbmi_match_end[order(match(fiadjbmi_match_end$chr_pos, X$snp)),]

length(fiadjbmi_match_end$chr_pos == X$snp) #perfect

#We can do this!

X$snp <- fiadjbmi_match_end$RSID
