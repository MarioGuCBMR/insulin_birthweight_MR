...
#This is code to run CAUSE for fiadjbmi -> FBW
...
summary(as.numeric(FBW$`EGG-frq`)) #Lots of NAs, but we the frequencies are OK.
...
summary(as.numeric(FBW_maf_no_NA$`EGG-frq`)) #CLEANED before
...
X <- gwas_merge(fiadjbmi_, FBW_, snp_name_cols = c("chr_pos_37", "chr_pos_37"), 
...
FBW_match_no_rsid <- FBW_[which(FBW_$chr_pos_37%in%fiadjbmi_match_no_rsid$chr_pos_37),] #WE HAVE THEM.

#Now let's order them and check which ones can we retrieve there.

FBW_match_no_rsid <- FBW_match_no_rsid[order(match(FBW_match_no_rsid$chr_pos_37, fiadjbmi_match_no_rsid$chr_pos_37)),]

length(which(FBW_match_no_rsid$chr_pos_37 == fiadjbmi_match_no_rsid$chr_pos_37)) #DONE.

#Let's be wary and get an index of all of those that have rsids:

index_rsid <- which(str_detect(FBW_match_no_rsid$rsID, "rs") == TRUE) #most of them!!

FBW_match_no_rsid_recovered <- FBW_match_no_rsid[index_rsid,]
fiadjbmi_match_no_rsid_recovered <- fiadjbmi_match_no_rsid[index_rsid,]

length(which(FBW_match_no_rsid_recovered$chr_pos_37 == fiadjbmi_match_no_rsid_recovered$chr_pos_37)) #DONE. 

fiadjbmi_match_no_rsid_recovered$SNP <- FBW_match_no_rsid_recovered$rsID

#AWESOME: now let's be wary with the others:

FBW_match_no_rsid_missed <- FBW_match_no_rsid[-index_rsid,]
fiadjbmi_match_no_rsid_missed <- fiadjbmi_match_no_rsid[-index_rsid,]

#Let's try with phenoscanner:

ps_results_1 <- phenoscanner::phenoscanner(FBW_match_no_rsid_missed$chr_pos_37[seq(1,100)])
ps_results_2 <- phenoscanner::phenoscanner(FBW_match_no_rsid_missed$chr_pos_37[seq(101,120)])
...
FBW_match_no_rsid_missed_recovered <- FBW_match_no_rsid_missed[which(FBW_match_no_rsid_missed$chr_pos_37%in%ps_end$hg19_coordinates),]
...
ps_end <- ps_end[order(match(ps_end$hg19_coordinates, FBW_match_no_rsid_missed_recovered$chr_pos_37)),]

length(which(ps_end$hg19_coordinates == FBW_match_no_rsid_missed_recovered$chr_pos_37))
...
FBW_match_no_rsid_missed_missed <- FBW_match_no_rsid_missed[which(!(FBW_match_no_rsid_missed$chr_pos_37%in%ps_end$hg19_coordinates)),]
...
saveRDS(X_end, file = "/home/projects/ku_00095/people/marure/BW_project/CODE/4_CAUSE/IR/fiadjbmi/FBW/dataframes/X_fiadjbmi_FBW.rds")
saveRDS(params, file = "/home/projects/ku_00095/people/marure/BW_project/CODE/4_CAUSE/IR/fiadjbmi/FBW/dataframes/params_fiadjbmi_FBW.rds")
...
saveRDS(top_vars, "/home/projects/ku_00095/people/marure/BW_project/CODE/4_CAUSE/IR/fiadjbmi/FBW/dataframes/pruned_fiadjbmi_FBW.res")
...
saveRDS(res, file = "/home/projects/ku_00095/people/marure/BW_project/CODE/4_CAUSE/IR/fiadjbmi/FBW/output/res_fiadjbmi_FBW.rds")

res_strict <- cause(X=X_end, variants = top_vars, param_ests = params, qalpha = 1, qbeta = 2)

saveRDS(res, file = "/home/projects/ku_00095/people/marure/BW_project/CODE/4_CAUSE/IR/fiadjbmi/FBW/output/res_fiadjbmi_FBW_strict.rds")
...
tiff("/home/projects/ku_00095/people/marure/BW_project/CODE/4_CAUSE/IR/fiadjbmi/FBW/output/res_fiadjbmi_FBW_plot.tiff", units="in", width=1200, height=500, res=300)
plot(res, type="data")
dev.off()
