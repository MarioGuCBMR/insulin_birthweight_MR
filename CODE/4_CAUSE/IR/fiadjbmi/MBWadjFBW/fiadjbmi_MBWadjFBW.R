...
#This is code to run CAUSE for fiadjbmi -> MBWadjFBW
...
X <- gwas_merge(fiadjbmi_, MBWadjFBW_, snp_name_cols = c("chr_pos", "chr_pos"), 
...
MBWadjFBW_match_no_rsid <- MBWadjFBW_[which(MBWadjFBW_$chr_pos%in%fiadjbmi_match_no_rsid$chr_pos),] #WE HAVE THEM.

#Now let's order them and check which ones can we retrieve there.

MBWadjFBW_match_no_rsid <- MBWadjFBW_match_no_rsid[order(match(MBWadjFBW_match_no_rsid$chr_pos, fiadjbmi_match_no_rsid$chr_pos)),]

length(which(MBWadjFBW_match_no_rsid$chr_pos == fiadjbmi_match_no_rsid$chr_pos)) #DONE.

#Let's be wary and get an index of all of those that have rsids:

index_rsid <- which(str_detect(MBWadjFBW_match_no_rsid$RSID, "rs") == TRUE) #most of them!!

MBWadjFBW_match_no_rsid_recovered <- MBWadjFBW_match_no_rsid[index_rsid,]
fiadjbmi_match_no_rsid_recovered <- fiadjbmi_match_no_rsid[index_rsid,]

length(which(MBWadjFBW_match_no_rsid_recovered$chr_pos == fiadjbmi_match_no_rsid_recovered$chr_pos)) #DONE. 

fiadjbmi_match_no_rsid_recovered$RSID <- MBWadjFBW_match_no_rsid_recovered$RSID

#AWESOME: now let's be wary with the others:

MBWadjFBW_match_no_rsid_missed <- MBWadjFBW_match_no_rsid[-index_rsid,]
fiadjbmi_match_no_rsid_missed <- fiadjbmi_match_no_rsid[-index_rsid,]

#Let's try with phenoscanner:

ps_results_1 <- phenoscanner::phenoscanner(MBWadjFBW_match_no_rsid_missed$chr_pos)
...
MBWadjFBW_match_no_rsid_missed_recovered <- MBWadjFBW_match_no_rsid_missed[which(MBWadjFBW_match_no_rsid_missed$chr_pos%in%ps_end$hg19_coordinates),]
...
ps_end <- ps_end[order(match(ps_end$hg19_coordinates, MBWadjFBW_match_no_rsid_missed_recovered$chr_pos)),]

length(which(ps_end$hg19_coordinates == MBWadjFBW_match_no_rsid_missed_recovered$chr_pos))
...
MBWadjFBW_match_no_rsid_missed_missed <- MBWadjFBW_match_no_rsid_missed[which(!(MBWadjFBW_match_no_rsid_missed$chr_pos%in%ps_end$hg19_coordinates)),]
