...
#This is code to run CAUSE for homair -> MBW
...
summary(as.numeric(MBW$`EGG-frq`)) #Lots of NAs, but we have frequencies that are a tad weird.
...
summary(as.numeric(MBW_maf_no_NA$`EGG-frq`)) #CLEANED

MBW_maf_no_NA <- MBW_maf_no_NA[which(as.numeric(MBW_maf_no_NA$`EGG-frq`) > 1),]
MBW_maf_no_NA <- MBW_maf_no_NA[which(as.numeric(MBW_maf_no_NA$`EGG-frq`) < 99),]
...
homair_missing <- homair_[which(homair_$chr_pos_37 == "chrNA:NA"),] #347

#Let's check if the rsids are either in the MBW rsIDs or in the merged rsIDs that they also share, which is supercool.

homair_missing_match_1 <- homair_missing[which(homair_missing$snp%in%MBW_$rsID),] #149!!!Quite many. That is great
MBW_missing_match_1 <- MBW_[which(MBW_$rsID%in%homair_missing$snp),]

#Now I understand, these are only available in build 38 or 18.
#Well that makes a lot of sense. 
#Let's see if we have some match with merged_rsID:

homair_missing_match_2 <- homair_missing[which(homair_missing$snp%in%MBW_$merged_rsID),] #0
MBW_missing_match_2 <- MBW_[which(MBW_$merged_rsID%in%homair_missing$snp),] #0
...
X <- gwas_merge(homair_not_missing, MBW_, snp_name_cols = c("chr_pos_37", "chr_pos_37"), 
...
X_missing <- gwas_merge(homair_missing_match_1, MBW_missing_match_1, snp_name_cols = c("SNP", "rsID"), 
                beta_hat_cols = c("effect", "Beta-A1"), 
                se_cols = c("stderr", "SE"), 
                A1_cols = c("effect_allele", "A1"), 
                A2_cols = c("other_allele", "A0"))
