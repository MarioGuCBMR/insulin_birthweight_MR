...
#This is code to run CAUSE for homair -> FBWadjMBW
...
#Let's check if the rsids are either in the FBWadjMBW rsIDs or in the merged rsIDs that they also share, which is supercool.

homair_missing_match_1 <- homair_missing[which(homair_missing$snp%in%FBWadjMBW_$RSID),] #4! Not too many.
FBWadjMBW_missing_match_1 <- FBWadjMBW_[which(FBWadjMBW_$RSID%in%homair_missing$snp),]

#Let's do the rest:

homair_missing_rest <- homair_missing[which(!(homair_missing$snp%in%homair_missing_match_1$snp)),]
...
index_merged_old <- which(merged_rs$old_rs%in%homair_missing_rest$snp) #Only 10! 
index_merged_new <- which(merged_rs$new_rs%in%homair_missing_rest$snp) #We have 708. That means that we have A LOT of duplicates. All of them were too new it seems!! 
...
which(swapping_snps_old_2_new$new_rs%in%FBWadjMBW_$RSID) #3 need to be changed.
which(swapping_snps_new_2_old$old_rs%in%FBWadjMBW_$RSID) #0: that means that we don't need to change these either.

mismatch <- swapping_snps_old_2_new[which(swapping_snps_old_2_new$new_rs%in%FBWadjMBW_$RSID),]

homair_missing_match_2 <- homair_missing_rest[which(homair_missing_rest$snp%in%mismatch$old_rs),]

homair_missing_match_2 <- homair_missing_match_2[order(match(homair_missing_match_2$snp, mismatch$old_rs)),]

length(homair_missing_match_2$snp == mismatch$old_rs) #perfect

homair_missing_match_2$snp <- mismatch$new_rs #CHANGED

FBWadjMBW_missing_match_2 <- FBWadjMBW_[which(FBWadjMBW_$RSID%in%homair_missing_match_2$snp),]
...
X <- gwas_merge(homair_not_missing, FBWadjMBW_, snp_name_cols = c("chr_pos_37", "chr_pos"), 
...
X_missing_1 <- gwas_merge(homair_missing_match_1, FBWadjMBW_missing_match_1, snp_name_cols = c("SNP", "RSID"), 
                beta_hat_cols = c("effect", "beta"), 
                se_cols = c("stderr", "se"), 
                A1_cols = c("effect_allele", "ea"), 
                A2_cols = c("other_allele", "nea"))

colnames(homair_missing_match_2) <- c("SNP", "effect_allele", "other_allele",  "maf", "effect", "stderr", "pvalue", "chr_pos_37", "CHR", "POS")

X_missing_2 <- gwas_merge(homair_missing_match_2, FBWadjMBW_missing_match_2, snp_name_cols = c("SNP", "RSID"), 
                        beta_hat_cols = c("effect", "beta"), 
                        se_cols = c("stderr", "se"), 
                        A1_cols = c("effect_allele", "ea"), 
                        A2_cols = c("other_allele", "nea"))
...
FBWadjMBW_match <- FBWadjMBW_[which(FBWadjMBW_$chr_pos%in%X$snp),]

#Let's check the rsID here.

FBWadjMBW_match <- FBWadjMBW_match[order(FBWadjMBW_match$RSID),]

head(FBWadjMBW_match, 100) #we do not have most of them
tail(FBWadjMBW_match) 
...
#We have to do the same for homairadjISI match 1:

homair_missing_match_1_match <- homair_missing_match_1[which(homair_missing_match_1$SNP%in%X_missing_1$snp),]
homair_missing_match_1_match <- homair_missing_match_1_match[order(match(homair_missing_match_1_match$SNP, X_missing_1$snp)),]

length(homair_missing_match_1_match$SNP == X_missing_1$snp) #all of them

X_missing_1$p1 <- homair_missing_match_1_match$pvalue

#And now we do the same for homairadjISI match 2:

homair_missing_match_2_match <- homair_missing_match_2[which(homair_missing_match_2$SNP%in%X_missing_2$snp),]
homair_missing_match_2_match <- homair_missing_match_2_match[order(match(homair_missing_match_2_match$SNP, X_missing_2$snp)),]

length(homair_missing_match_2_match$SNP == X_missing_2$snp) #all of them

X_missing_2$p1 <- homair_missing_match_2_match$pvalue
