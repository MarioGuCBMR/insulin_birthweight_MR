...
#This is code to run CAUSE for homair -> Father
...
homair_missing_match_1 <- homair_missing[which(homair_missing$snp%in%Father_$rsID),] #150!!! We find one more than with the rest. Probably cuz there is no EGG data here to filter with.
Father_missing_match_1 <- Father_[which(Father_$rsID%in%homair_missing$snp),]

#Now I understand, these are only available in build 38 or 18.
#Well that makes a lot of sense. 
#Let's see if we have some match with merged_rsID:

homair_missing_match_2 <- homair_missing[which(homair_missing$snp%in%Father_$merged_rsID),] #0
Father_missing_match_2 <- Father_[which(Father_$merged_rsID%in%homair_missing$snp),] #0
...
X <- gwas_merge(homair_not_missing, Father_, snp_name_cols = c("chr_pos_37", "chr_pos_37"), 
...
X_missing <- gwas_merge(homair_missing_match_1, Father_missing_match_1, snp_name_cols = c("SNP", "rsID"), 
                beta_hat_cols = c("effect", "Beta-A1"), 
                se_cols = c("stderr", "SE"), 
                A1_cols = c("effect_allele", "A1"), 
                A2_cols = c("other_allele", "A0"))
