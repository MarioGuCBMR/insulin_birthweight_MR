...
#This is code to run CAUSE for CIR -> MBW
...
CIR_missing_match_1 <- CIR_missing[which(CIR_missing$snp%in%MBW_$rsID),] #149!!!Quite many. That is great
MBW_missing_match_1 <- MBW_[which(MBW_$rsID%in%CIR_missing$snp),]

#Now I understand, these are only available in build 38 or 18.
#Well that makes a lot of sense. 
#Let's see if we have some match with merged_rsID:

CIR_missing_match_2 <- CIR_missing[which(CIR_missing$snp%in%MBW_$merged_rsID),] #0
MBW_missing_match_2 <- MBW_[which(MBW_$merged_rsID%in%CIR_missing$snp),] #0
...
X <- gwas_merge(CIR_not_missing, MBW_, snp_name_cols = c("chr_pos_37", "chr_pos_37"), 
...
X_missing <- gwas_merge(CIR_missing_match_1, MBW_missing_match_1, snp_name_cols = c("SNP", "rsID"), 
                beta_hat_cols = c("effect", "Beta-A1"), 
                se_cols = c("stderr", "SE"), 
                A1_cols = c("effect_allele", "A1"), 
                A2_cols = c("other_allele", "A0"))
