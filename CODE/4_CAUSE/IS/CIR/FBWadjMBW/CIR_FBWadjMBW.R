...
#This is code to run CAUSE for CIR -> FBWadjMBW
...
FBWadjMBW_maf_no_NA <- FBWadjMBW_maf_no_NA[which(as.numeric(FBWadjMBW_maf_no_NA$`EGG-frq`) > 1),]
FBWadjMBW_maf_no_NA <- FBWadjMBW_maf_no_NA[which(as.numeric(FBWadjMBW_maf_no_NA$`EGG-frq`) < 99),]

summary(as.numeric(FBWadjMBW_maf_no_NA$`EGG-frq`)) #CLEANED
...
CIR_missing_match_1 <- CIR_missing[which(CIR_missing$snp%in%FBWadjMBW_$RSID),] #4! Not too many.
FBWadjMBW_missing_match_1 <- FBWadjMBW_[which(FBWadjMBW_$RSID%in%CIR_missing$snp),]

#Let's do the rest:

CIR_missing_rest <- CIR_missing[which(!(CIR_missing$snp%in%CIR_missing_match_1$snp)),]
...
index_merged_old <- which(merged_rs$old_rs%in%CIR_missing_rest$snp) #Only 6! 
index_merged_new <- which(merged_rs$new_rs%in%CIR_missing_rest$snp) #We have 714. That means that we have A LOT of duplicates. All of them were too new it seems!! 
...
CIR_missing_match_2 <- CIR_missing_rest[which(CIR_missing_rest$snp == mismatch$old_rs),]

CIR_missing_match_2$snp <- mismatch$new_rs #CHANGED

FBWadjMBW_missing_match_2 <- FBWadjMBW_[which(FBWadjMBW_$RSID%in%CIR_missing_match_2$snp),]
...
X <- gwas_merge(CIR_not_missing, FBWadjMBW_, snp_name_cols = c("chr_pos_37", "chr_pos"), 
...
X_missing_1 <- gwas_merge(CIR_missing_match_1, FBWadjMBW_missing_match_1, snp_name_cols = c("SNP", "RSID"), 
                beta_hat_cols = c("effect", "beta"), 
                se_cols = c("stderr", "se"), 
                A1_cols = c("effect_allele", "ea"), 
                A2_cols = c("other_allele", "nea"))

colnames(CIR_missing_match_2) <- c("SNP", "effect_allele", "other_allele",  "maf", "effect", "stderr", "pvalue", "chr_pos_37", "CHR", "POS")

X_missing_2 <- gwas_merge(CIR_missing_match_2, FBWadjMBW_missing_match_2, snp_name_cols = c("SNP", "RSID"), 
                        beta_hat_cols = c("effect", "beta"), 
                        se_cols = c("stderr", "se"), 
                        A1_cols = c("effect_allele", "ea"), 
                        A2_cols = c("other_allele", "nea"))
...
length(which(CIR_match$chr_pos_37 == X$snp)) #all of them
