...
#This is code to run CAUSE for CIR -> MBWadjFBW
...
CIR_missing_match_1 <- CIR_missing[which(CIR_missing$snp%in%MBWadjFBW_$RSID),] #4! Not too many.
MBWadjFBW_missing_match_1 <- MBWadjFBW_[which(MBWadjFBW_$RSID%in%CIR_missing$snp),]
...
index_merged_new <- which(merged_rs$new_rs%in%CIR_missing_rest$snp) #We have 705. That means that we have A LOT of duplicates. All of them were too new it seems!! 
...
MBWadjFBW_missing_match_2 <- MBWadjFBW_[which(MBWadjFBW_$RSID%in%CIR_missing_match_2$snp),]
...
X <- gwas_merge(CIR_not_missing, MBWadjFBW_, snp_name_cols = c("chr_pos_37", "chr_pos"), 
...
X_missing_1 <- gwas_merge(CIR_missing_match_1, MBWadjFBW_missing_match_1, snp_name_cols = c("SNP", "RSID"), 
                beta_hat_cols = c("effect", "beta"), 
                se_cols = c("stderr", "se"), 
                A1_cols = c("effect_allele", "ea"), 
                A2_cols = c("other_allele", "nea"))
...
X_missing_2 <- gwas_merge(CIR_missing_match_2, MBWadjFBW_missing_match_2, snp_name_cols = c("SNP", "RSID"), 
                        beta_hat_cols = c("effect", "beta"), 
                        se_cols = c("stderr", "se"), 
                        A1_cols = c("effect_allele", "ea"), 
                        A2_cols = c("other_allele", "nea"))
