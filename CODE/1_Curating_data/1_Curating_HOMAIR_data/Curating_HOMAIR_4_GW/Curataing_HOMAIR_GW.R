...
#This is a quick code to check how much do we need to clean the HOMA_IR stratified data.
...
HOMA_IR <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/RAW_DATA/Glycemic_traits_raw_data/MAGIC_ln_HOMA-IR.txt") 

#Let's check what columns do we have:

HOMA_IR <- HOMA_IR[order(HOMA_IR$snp),]

head(HOMA_IR) #perfect
tail(HOMA_IR) #perfect
...
#The HOMA_IR data has some of the SNPs without allele frequency.
#We cannot know if the imputation was done correctly...
#Hence, we are gonna remove them.

summary(HOMA_IR$maf) #perfect.

HOMA_IR_maf <- HOMA_IR[which(HOMA_IR$maf > 0.01),]

summary(HOMA_IR_maf$maf) #perfect.

#Now the INFO, do we have that data?

colnames(HOMA_IR_maf) #we do not. 
...
bmi_match <- bmi[which(bmi$RSID%in%HOMA_IR_maf$snp),] #most of them:

HOMA_IR_Pulit <- HOMA_IR_maf[which(HOMA_IR_maf$snp%in%bmi_match$RSID),]
...
bmi_match <- bmi_match[order(match(bmi_match$RSID, HOMA_IR_Pulit$snp)),]

#Let's check if we did it properly:

length(which(bmi_match$RSID != HOMA_IR_Pulit$snp)) #perfect.
length(which(bmi_match$RSID == HOMA_IR_Pulit$snp)) #perfect.

#We can match the chr_pos:

HOMA_IR_Pulit$chr_pos_37 <- paste("chr", bmi_match$CHR, ":", bmi_match$POS, sep = "")
...
HOMA_IR_missing <- HOMA_IR_maf[which(!(HOMA_IR_maf$snp%in%bmi_match$RSID)),]

#Let's see how many of them can be recovered from SNP Nexus:

for_nexus <- HOMA_IR_missing
...
write.table(for_nexus, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CURATED_DATA/SNPNexus/INPUT/HOMA_IR/HOMA_IR_combined_4_Nexus.txt", sep = " ", quote = FALSE, col.names = FALSE, row.names = FALSE)

####################
#Let's recover them#
####################

recovered_snps <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CURATED_DATA/SNPNexus/OUTPUT/HOMA_IR/HOMA_IR_combined_in_Nexus.txt")
...
HOMA_IR_recovered <- HOMA_IR_missing[which(HOMA_IR_missing$snp%in%recovered_snps$dbSNP),]

recovered_snps <- recovered_snps[order(match(recovered_snps$dbSNP, HOMA_IR_recovered$snp)),]

#Doing some checkity checks:

which(recovered_snps$dbSNP != HOMA_IR_recovered$MarkerName)

head(recovered_snps$dbSNP)
head(HOMA_IR_recovered$snp)
...
HOMA_IR_recovered$chr_pos_37 <- recovered_snps$chr_pos

###############################
#Let's check the missing ones:#
###############################

HOMA_IR_missing_2 <- HOMA_IR_missing[which(!(HOMA_IR_missing$snp%in%recovered_snps$dbSNP)),]

check <- myvariant::queryVariants(HOMA_IR_missing_2$snp)
...
check_df <- check_df[order(match(check_df$query, HOMA_IR_missing_2$snp)),]

length(which(check_df$query == HOMA_IR_missing_2$snp)) #all of them

HOMA_IR_missing_2$chr_pos_37 <- check_df$chr_pos #349 still missing.
...
HOMA_IR_end <- rbind(HOMA_IR_Pulit, HOMA_IR_recovered, HOMA_IR_missing_2) #PERFECT number. It matches the data perfectily.

dim(HOMA_IR_end)
dim(HOMA_IR_maf)

fwrite(HOMA_IR_end, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CURATED_DATA/Data_4_GW_analysis/HOMA_IR/HOMA_IR_combined_Curated.txt")
