...
FBW_rsid <- FBW[which(str_detect(FBW$rsID, "rs") == TRUE),]
FBW_rsid_fail <- FBW[which(str_detect(FBW$rsID, "rs") == FALSE),]

#Now let's do some checks...

length(which(FBW_rsid$rsID%in%ukbb$rsid)) #(1028668)/(1029876) Most of them!! but we are missing some fellas.
length(which(FBW_rsid$merged_rsID%in%ukbb$rsid)) #enough to be OK.
...
colnames(FBW_4_hdl) <- c("SNP", "A1", "A2", "N", "b", "se")

FBW_4_hdl$SNP <- ifelse(str_detect(FBW_4_hdl$SNP, "rs") == FALSE, "-", FBW_4_hdl$SNP) #usually we expect them to have mean 0 and error of 1 so, we are gonna set it up like that.
FBW_4_hdl$se <- ifelse(is.na(FBW_4_hdl$se) == TRUE, 1, FBW_4_hdl$se) #usually we expect them to have mean 0 and error of 1 so, we are gonna set it up like that.

fwrite(FBW_4_hdl, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CODE/2_Genetic_Correlations/MUNGED_DATA/FBW_4_HDL.txt", sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)
