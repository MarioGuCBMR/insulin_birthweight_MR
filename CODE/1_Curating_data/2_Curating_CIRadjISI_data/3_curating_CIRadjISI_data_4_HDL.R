...
#This is code to clean CIRadjISI data for GC (HDL).
...
length(which(CIRadjISI$snp%in%ukbb$rsid)) #not enough for this reference panel, but enough for the other one.
...
CIRadjISI_4_hdl <- CIRadjISI %>%
  select(snp, effect_allele, other_allele, N, effect, stderr)

colnames(CIRadjISI_4_hdl) <- c("SNP", "A1", "A2", "N", "b", "se")

fwrite(CIRadjISI_4_hdl, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CODE/2_Genetic_Correlations/MUNGED_DATA/CIRadjISI_4_HDL.txt", sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)
