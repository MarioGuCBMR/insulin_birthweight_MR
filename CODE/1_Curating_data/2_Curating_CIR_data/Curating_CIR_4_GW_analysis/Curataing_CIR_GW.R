...
#This is a quick code to check how much do we need to clean the CIR stratified data.
...
#The CIR data has some of the SNPs without allele frequency.
#We cannot know if the imputation was done correctly...
#Hence, we are gonna remove them.

summary(CIR$maf) #perfect.

CIR_maf <- CIR[which(CIR$maf > 0.01),]

summary(CIR_maf$maf) #perfect.

#Now the INFO, do we have that data?

colnames(CIR_maf) #we do not. 
...
bmi_match <- bmi[which(bmi$RSID%in%CIR_maf$snp),] #most of them:

CIR_Pulit <- CIR_maf[which(CIR_maf$snp%in%bmi_match$RSID),]
...
bmi_match <- bmi_match[order(match(bmi_match$RSID, CIR_Pulit$snp)),]

#Let's check if we did it properly:

length(which(bmi_match$RSID != CIR_Pulit$snp)) #perfect.
length(which(bmi_match$RSID == CIR_Pulit$snp)) #perfect.

#We can match the chr_pos:

CIR_Pulit$chr_pos_37 <- paste("chr", bmi_match$CHR, ":", bmi_match$POS, sep = "")
...
CIR_missing <- CIR_maf[which(!(CIR_maf$snp%in%bmi_match$RSID)),]

#Let's see how many of them can be recovered from SNP Nexus:

for_nexus <- CIR_missing
...
write.table(for_nexus, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CURATED_DATA/SNPNexus/INPUT/CIR/CIR_combined_4_Nexus.txt", sep = " ", quote = FALSE, col.names = FALSE, row.names = FALSE)

####################
#Let's recover them#
####################

recovered_snps <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CURATED_DATA/SNPNexus/OUTPUT/CIR/CIR_combined_in_Nexus.txt")
...
CIR_recovered <- CIR_missing[which(CIR_missing$snp%in%recovered_snps$dbSNP),]

recovered_snps <- recovered_snps[order(match(recovered_snps$dbSNP, CIR_recovered$snp)),]

#Doing some checkity checks:

which(recovered_snps$dbSNP != CIR_recovered$MarkerName)

head(recovered_snps$dbSNP)
head(CIR_recovered$snp)
...
CIR_recovered$chr_pos_37 <- recovered_snps$chr_pos

###############################
#Let's check the missing ones:#
###############################

CIR_missing_2 <- CIR_missing[which(!(CIR_missing$snp%in%recovered_snps$dbSNP)),]

check <- myvariant::queryVariants(CIR_missing_2$snp)
...
check_df <- check_df[order(match(check_df$query, CIR_missing_2$snp)),]

length(which(check_df$query == CIR_missing_2$snp)) #all of them

CIR_missing_2$chr_pos_37 <- check_df$chr_pos #347 still missing.
...
CIR_end <- rbind(CIR_Pulit, CIR_recovered, CIR_missing_2) #PERFECT number. It matches the data perfectily.

dim(CIR_end)
dim(CIR_maf)

fwrite(CIR_end, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CURATED_DATA/Data_4_GW_analysis/CIR/CIR_combined_Curated.txt")
