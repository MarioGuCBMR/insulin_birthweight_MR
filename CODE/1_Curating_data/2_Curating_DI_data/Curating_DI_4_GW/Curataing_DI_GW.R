...
#This is a quick code to check how much do we need to clean the DI stratified data.
...
#The DI data has some of the SNPs without allele frequency.
#We cannot know if the imputation was done correctly...
#Hence, we are gonna remove them.

summary(DI$maf) #perfect.

DI_maf <- DI[which(DI$maf > 0.01),]

summary(DI_maf$maf) #perfect.

#Now the INFO, do we have that data?

colnames(DI_maf) #we do not. 
...
bmi_match <- bmi[which(bmi$RSID%in%DI_maf$snp),] #most of them:

DI_Pulit <- DI_maf[which(DI_maf$snp%in%bmi_match$RSID),]
...
bmi_match <- bmi_match[order(match(bmi_match$RSID, DI_Pulit$snp)),]

#Let's check if we did it properly:

length(which(bmi_match$RSID != DI_Pulit$snp)) #perfect.
length(which(bmi_match$RSID == DI_Pulit$snp)) #perfect.

#We can match the chr_pos:

DI_Pulit$chr_pos_37 <- paste("chr", bmi_match$CHR, ":", bmi_match$POS, sep = "")
...
DI_missing <- DI_maf[which(!(DI_maf$snp%in%bmi_match$RSID)),]

#Let's see how many of them can be recovered from SNP Nexus:

for_nexus <- DI_missing
...
write.table(for_nexus, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CURATED_DATA/SNPNexus/INPUT/DI/DI_combined_4_Nexus.txt", sep = " ", quote = FALSE, col.names = FALSE, row.names = FALSE)

####################
#Let's recover them#
####################

recovered_snps <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CURATED_DATA/SNPNexus/OUTPUT/DI/DI_combined_in_Nexus.txt")
...
DI_recovered <- DI_missing[which(DI_missing$snp%in%recovered_snps$dbSNP),]

recovered_snps <- recovered_snps[order(match(recovered_snps$dbSNP, DI_recovered$snp)),]

#Doing some checkity checks:

which(recovered_snps$dbSNP != DI_recovered$MarkerName)

head(recovered_snps$dbSNP)
head(DI_recovered$snp)
...
DI_recovered$chr_pos_37 <- recovered_snps$chr_pos

###############################
#Let's check the missing ones:#
###############################

DI_missing_2 <- DI_missing[which(!(DI_missing$snp%in%recovered_snps$dbSNP)),]

check <- myvariant::queryVariants(DI_missing_2$snp)
...
check_df <- check_df[order(match(check_df$query, DI_missing_2$snp)),]

length(which(check_df$query == DI_missing_2$snp)) #all of them

DI_missing_2$chr_pos_37 <- check_df$chr_pos #349 still missing.
...
DI_end <- rbind(DI_Pulit, DI_recovered, DI_missing_2) #PERFECT number. It matches the data perfectily.

dim(DI_end)
dim(DI_maf)

fwrite(DI_end, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CURATED_DATA/Data_4_GW_analysis/DI/DI_combined_Curated.txt")
