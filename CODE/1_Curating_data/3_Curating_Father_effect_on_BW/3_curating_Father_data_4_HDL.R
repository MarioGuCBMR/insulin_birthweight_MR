...
Father_rsid <- Father[which(str_detect(Father$rsID, "rs") == TRUE),]
Father_rsid_fail <- Father[which(str_detect(Father$rsID, "rs") == FALSE),]

#Now let's do some checks...

length(which(Father_rsid$rsID%in%ukbb$rsid)) #(1015655)/(1029876) Most of them!! but we are missing some fellas.
length(which(Father_rsid$merged_rsID%in%ukbb$rsid)) #(1015655+79)/(1029876) Most of them!! but we are missing some fellas.
...
colnames(Father_4_hdl) <- c("SNP", "A1", "A2", "N", "b", "se")

Father_4_hdl$SNP <- ifelse(str_detect(Father_4_hdl$SNP, "rs") == FALSE, "-", Father_4_hdl$SNP) #usually we expect them to have mean 0 and error of 1 so, we are gonna set it up like that.
Father_4_hdl$se <- ifelse(is.na(Father_4_hdl$se) == TRUE, 1, Father_4_hdl$se) #usually we expect them to have mean 0 and error of 1 so, we are gonna set it up like that.

fwrite(Father_4_hdl, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CODE/2_Genetic_Correlations/MUNGED_DATA/Father_4_HDL.txt", sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)
