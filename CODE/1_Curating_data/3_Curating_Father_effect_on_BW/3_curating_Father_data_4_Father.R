...
#This is code to clean Father data for GC (HDL).
...
Father$eaf <- as.numeric(Father$`IS-frq`)/100

Father$maf <- ifelse(Father$eaf > 0.5, 1-Father$eaf, Father$eaf)

Father_4_ldsc <- Father %>%
  select("rsID", "Beta-A1", "A1", "A0", "maf", "P", "IS-info")

colnames(Father_4_ldsc) <- c("SNP", "BETA", "A1", "A2", "MAF", "P", "INFO")

#fwrite(Father_4_ldsc, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CODE/2_Genetic_Correlations/DATA_4_MUNGING/Father_4_LDSC.txt", sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE, na = "-")

#########################
#Let's do some checks...#
#########################

check <- Father_4_ldsc[which(Father_4_ldsc$SNP != ""),]
check <- check[which(is.na(check$MAF) == FALSE),]
check <- check[which(is.na(check$BETA) == FALSE),]

fwrite(check, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CODE/2_Genetic_Correlations/DATA_4_MUNGING/Father_4_LDSC.txt", sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE, na = "-")
...
python munge_sumstats.py --sumstats "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CODE/2_Genetic_Correlations/DATA_4_MUNGING/Father_4_LDSC.txt" --N 60134 --out "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CODE/2_Genetic_Correlations/MUNGED_DATA/Father_4_LDSC.txt" --merge-alleles w_hm3.snplist
