...
#This is code to clean fiadjbmi data for GC (HDL).
...
length(which(fiadjbmi$RSID%in%ukbb$rsid)) #not enough for this reference panel, but enough for the other one.

######################################################
#PERFECT: LET'S GO WITH SELECTING THE COLUMNS FOR HDL#
######################################################

fiadjbmi_4_hdl <- fiadjbmi %>%
  select(RSID, effect_allele, other_allele, sample_size, beta, standard_error)

colnames(fiadjbmi_4_hdl) <- c("SNP", "A1", "A2", "N", "b", "se")

fwrite(fiadjbmi_4_hdl, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Hermina_and_Mario/IR_BW_AIM4/CODE/2_Genetic_Correlations/MUNGED_DATA/fiadjbmi_4_HDL.txt", sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)
